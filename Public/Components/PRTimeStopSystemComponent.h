 ...
 */
UCLASS()
class PROJECTREPLICA_API UPRTimeStopSystemComponent : public UActorComponent
{
	GENERATED_BODY()
...
private:
	/** 디버그 실행을 나타내는 변수입니다. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TimeStop", meta = (AllowPrivateAccess = "true"))
	bool bDebug;
		
	/** TimeStop의 실행을 나타내는 변수입니다. */
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "TimeStop", meta = (AllowPrivateAccess = "true"))
	bool bActivate;

	/** TimeStop에 영향을 받는 AI 캐릭터들입니다. */
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "TimeStop", meta = (AllowPrivateAccess = "true"))
	TArray<APRAICharacter*> TimeStopActiveAICharacters;
